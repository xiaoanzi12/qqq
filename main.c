...
{
    // 配置P1口为推挽输出模式（STC15特有）
    // P1M1和P1M0控制P1口的输出模式
    // P1M1=0, P1M0=0: 准双向口（默认）
    // P1M1=0, P1M0=1: 推挽输出
    // P1M1=1, P1M0=0: 高阻输入
    // P1M1=1, P1M0=1: 开漏输出
    P1M1 = 0x00;  // P1口配置寄存器1
...
void main(void)
{
    ...
    while(1)
    {
        ...
        /* 方法二：使用移位法实现流水灯（可选，注释掉方法一后启用）
        led_pattern = 0xFE;  // 初始化：LED0点亮
        for(i = 0; i < 8; i++)
        {
            P1 = led_pattern;        // 输出LED模式到P1口
            Delay_ms(DELAY_MS);      // 延迟，控制流水速度
 ...
 * 
 * 3. 延迟控制：
 *    使用软件延迟实现流水速度控制，可通过修改DELAY_MS调整
 * 
 * 4. 扩展功能建议：
 *    - 双向流水：LED从0到7，再从7到0
 *    - 多种模式：单个流动、多个流动、呼吸灯等
 *    - 速度可调：通过按键或定时器调整流水速度
 *    - 使用定时器：用定时器中断替代软件延迟，提高精度
 * 
 ******************************************************************************/
